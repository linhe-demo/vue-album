    ...
    <el-table
      ...
      <el-table-column prop="id" label="食物名" align="center">
        <template v-slot="scope" class="specialColor">
          <span style="color: #adadad;font-weight: bold;font-size: 11px;">{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="reason" label="不可吃原因" align="center" :cell-class-name="cellClassName">
        <template v-slot="scope" class="specialColor">
          <span @click="openDetail(scope.row.reason)" class="cell-text">{{ scope.row.reason }}</span>
        </template>
      </el-table-column>
    </el-table>
  </div>
